...
STYLE_PRESETS = {
    ...
    "清透日系生活绘本": (
        "纯白、无纹理的干净数字画布，使用纤细清晰、保留极轻手绘抖动的黑灰墨线；"
        "外轮廓明确但不粗重，内部只用极少短线表现发丝、衣褶和地面接触，保持现代日常漫画插画的清爽线稿。"
        "人物采用自然但可爱的生活绘本比例，头部轻度放大、身体修长简化、手脚结构清楚；"
        "眼睛为克制的小型黑色椭圆并带微小白色高光，鼻口短小，面部圆润，表情温柔清楚，禁止写实衰老刻画和夸张萌系大眼。"
        "使用干净数字淡彩与极轻彩铅触感：低饱和卡其、灰绿、芥末黄、砖红、藏青、米白和自然肤色平涂；"
        "颜色透明轻盈但边界整洁，仅在衣物内部保留很淡的细笔触，皮肤通透并带轻微粉色脸颊，几乎不画体积阴影。"
        "每幅画只保留当前叙事所需的主体和物件，背景以少量细灰线提示空间，大面积纯白留白，构图像清洁的现代生活漫画单页。"
        "内容边界：人物身份、数量、年龄、外貌、服装、动作、关系、道具与场景只能来自当前分镜，绝不继承风格示例图中的具体内容。"
        "禁止泛黄纸纹、复古乡村绘本、颗粒水彩铺底、写实老人皱纹、厚涂、油画、3D、摄影写实、高饱和颜色、强烈明暗、"
        "赛璐璐硬阴影、复杂满铺背景、文字、Logo、水印和装饰边框。"
    ),
}
