...
STYLE_PRESETS = {
    ...
    "清透日系生活绘本": (
        "纯白或极浅暖白的干净画布，使用纤细、稳定且保留轻微手绘抖动的黑灰墨线；"
        "轮廓有克制的粗细变化，只用稀疏短线表现发丝、衣褶、结构与地面接触，不使用粗重漫画描边。"
        "人物采用自然且略微简化的生活绘本比例，头部仅轻度放大，手脚结构清楚；"
        "五官小巧克制，以小型黑色椭圆眼、极少高光、短线鼻口和含蓄表情传递情绪，禁止夸张大眼和统一幼童比例。"
        "上色呈透明水彩叠加轻薄彩铅的质感，以低饱和暖棕、卡其、灰绿、芥末黄、砖红、藏青、米白与自然肤色为主；"
        "色块轻盈并保留细微纸笔纹理与少量不均匀，光影几乎省略，仅用一层很浅的冷灰或同色阴影区分体积。"
        "每幅画只保留当前叙事所需的主体和物件，背景用少量细线提示空间，大面积留白，构图完整稳定，像安静的日常生活绘本单页。"
        "内容边界：人物身份、数量、年龄、外貌、服装、动作、关系、道具与场景只能来自当前分镜，绝不继承风格示例图中的具体内容。"
        "禁止动漫赛璐璐、Q版、厚涂、油画、3D、摄影写实、高饱和颜色、强烈明暗、渐变光效、复杂满铺背景、文字、Logo、水印和装饰边框。"
    ),
}
